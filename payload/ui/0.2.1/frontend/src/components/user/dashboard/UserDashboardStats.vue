<template>
  <!-- Row 1: Core Stats -->
  <div class="user-stat-grid grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
    <!-- Balance -->
    <div v-if="!isSimple" class="card user-stat-card p-4">
      <div class="user-stat-row flex min-w-0 items-center gap-3">
        <div class="user-stat-icon flex-shrink-0 rounded-lg bg-emerald-100 p-2 dark:bg-emerald-900/30">
        ...
        <div class="user-stat-body min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.balance') }}</p>
          <p class="user-stat-value text-xl font-bold text-emerald-600 dark:text-emerald-400">${{ formatBalance(balance) }}</p>
          <p class="user-stat-detail text-xs text-gray-500 dark:text-gray-400">{{ t('common.available') }}</p>
        </div>
      </div>
    </div>

    <!-- API Keys -->
    <div class="card user-stat-card p-4">
      <div class="user-stat-row flex min-w-0 items-center gap-3">
        <div class="user-stat-icon flex-shrink-0 rounded-lg bg-blue-100 p-2 dark:bg-blue-900/30">
          <Icon name="key" size="md" class="text-blue-600 dark:text-blue-400" :stroke-width="2" />
        </div>
        <div class="user-stat-body min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.apiKeys') }}</p>
          <p class="user-stat-value text-xl font-bold text-gray-900 dark:text-white">{{ stats?.total_api_keys || 0 }}</p>
          <p class="user-stat-detail text-xs text-green-600 dark:text-green-400">{{ stats?.active_api_keys || 0 }} {{ t('common.active') }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card user-stat-card p-4">
      <div class="user-stat-row flex min-w-0 items-center gap-3">
        <div class="user-stat-icon flex-shrink-0 rounded-lg bg-green-100 p-2 dark:bg-green-900/30">
          <Icon name="chart" size="md" class="text-green-600 dark:text-green-400" :stroke-width="2" />
        </div>
        <div class="user-stat-body min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayRequests') }}</p>
          <p class="user-stat-value text-xl font-bold text-gray-900 dark:text-white">{{ stats?.today_requests || 0 }}</p>
          <p class="user-stat-detail text-xs text-gray-500 dark:text-gray-400">{{ t('common.total') }}: {{ formatNumber(stats?.total_requests || 0) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card user-stat-card p-4">
      <div class="user-stat-row flex min-w-0 items-center gap-3">
        <div class="user-stat-icon flex-shrink-0 rounded-lg bg-purple-100 p-2 dark:bg-purple-900/30">
          <Icon name="dollar" size="md" class="text-purple-600 dark:text-purple-400" :stroke-width="2" />
        </div>
        <div class="user-stat-body min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</p>
          <p class="user-stat-value text-xl font-bold text-gray-900 dark:text-white">
          ...
          <p class="user-stat-detail text-xs">
  ...
  <div class="user-stat-grid grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <div class="card user-stat-card p-4">
      <div class="user-stat-row flex min-w-0 items-center gap-3">
        <div class="user-stat-icon flex-shrink-0 rounded-lg bg-amber-100 p-2 dark:bg-amber-900/30">
          <Icon name="cube" size="md" class="text-amber-600 dark:text-amber-400" :stroke-width="2" />
        </div>
        <div class="user-stat-body min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayTokens') }}</p>
          <p class="user-stat-value text-xl font-bold text-gray-900 dark:text-white">{{ formatTokens(stats?.today_tokens || 0) }}</p>
          <p class="user-stat-detail text-xs text-gray-500 dark:text-gray-400">{{ t('dashboard.input') }}: {{ formatTokens(stats?.today_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.today_output_tokens || 0) }} / {{ t('dashboard.cache') }}: {{ formatTokens((stats?.today_cache_creation_tokens || 0) + (stats?.today_cache_read_tokens || 0)) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card user-stat-card p-4">
      <div class="user-stat-row flex min-w-0 items-center gap-3">
        <div class="user-stat-icon flex-shrink-0 rounded-lg bg-indigo-100 p-2 dark:bg-indigo-900/30">
          <Icon name="database" size="md" class="text-indigo-600 dark:text-indigo-400" :stroke-width="2" />
        </div>
        <div class="user-stat-body min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.totalTokens') }}</p>
          <p class="user-stat-value text-xl font-bold text-gray-900 dark:text-white">{{ formatTokens(stats?.total_tokens || 0) }}</p>
          <p class="user-stat-detail text-xs text-gray-500 dark:text-gray-400">{{ t('dashboard.input') }}: {{ formatTokens(stats?.total_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.total_output_tokens || 0) }} / {{ t('dashboard.cache') }}: {{ formatTokens((stats?.total_cache_creation_tokens || 0) + (stats?.total_cache_read_tokens || 0)) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card user-stat-card p-4">
      <div class="user-stat-row flex min-w-0 items-center gap-3">
        <div class="user-stat-icon flex-shrink-0 rounded-lg bg-violet-100 p-2 dark:bg-violet-900/30">
          <Icon name="bolt" size="md" class="text-violet-600 dark:text-violet-400" :stroke-width="2" />
        </div>
        <div class="user-stat-body min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.performance') }}</p>
          <div class="flex min-w-0 items-baseline gap-2">
            <p class="user-stat-value min-w-0 text-xl font-bold text-gray-900 dark:text-white">{{ formatTokens(stats?.rpm || 0) }}</p>
            <span class="flex-shrink-0 text-xs text-gray-500 dark:text-gray-400">RPM</span>
          </div>
          <div class="flex min-w-0 items-baseline gap-2">
            <p class="user-stat-detail min-w-0 text-sm font-semibold text-violet-600 dark:text-violet-400">{{ formatTokens(stats?.tpm || 0) }}</p>
            <span class="flex-shrink-0 text-xs text-gray-500 dark:text-gray-400">TPM</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="card user-stat-card p-4">
      <div class="user-stat-row flex min-w-0 items-center gap-3">
        <div class="user-stat-icon flex-shrink-0 rounded-lg bg-rose-100 p-2 dark:bg-rose-900/30">
          <Icon name="clock" size="md" class="text-rose-600 dark:text-rose-400" :stroke-width="2" />
        </div>
        <div class="user-stat-body min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.avgResponse') }}</p>
          <p class="user-stat-value text-xl font-bold text-gray-900 dark:text-white">{{ formatDuration(stats?.average_duration_ms || 0) }}</p>
          <p class="user-stat-detail text-xs text-gray-500 dark:text-gray-400">{{ t('dashboard.averageTime') }}</p>
        </div>
      </div>
    </div>
  </div>
